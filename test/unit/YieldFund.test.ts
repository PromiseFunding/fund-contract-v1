...
!(network.name == "localhost")
    ...
    : describe("YieldFund Unit Tests (On AAVE Sandbox)", function () {
        ...
        const fundValue = 1
        let yieldFundContract: YieldFundAAVE, yieldFund: YieldFundAAVE, assetToken: any, assetTokenContract: any
        ...
        describe("Owner Tests", function () {
            it("fails when a non owner tries to withdraw proceeds", async function () {
                yieldFund = yieldFundContract.connect(user)
                await expect(yieldFund.withdrawProceeds(1)).to.be.revertedWith(
                    "Ownable: caller is not the owner"
                )
            })
            it("fails when a non owner tries to withdraw proceeds", async function () {
                yieldFund = yieldFundContract.connect(user)
                await expect(yieldFund.withdrawProceeds(1)).to.be.revertedWith("Ownable: caller is not the owner")
            })
        })
